...
# Function to directly run the classifier from command line
def main():
    if len(sys.argv) < 4:
        print("Usage: python vector_classifier.py <model_path> <category_embeddings_path> <text_to_classify>")
        sys.exit(1)
    
    model_path = sys.argv[1]
    category_embeddings_path = sys.argv[2]
    ...
    cleaned_text = clean_text(text)
    result = classifier.predict_categories(cleaned_text, text)
